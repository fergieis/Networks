...
def date2t(curr_date):
	...
	diff = diff.days + diff.seconds//43200,
	return diff
...
enddate = dict(zip(info.State, (info.endT//2)))
citylist=data.City
...
visits = pd.date_range(start_date, end_date, freq="24H")
...
for i,s in enumerate(states):
	...
	for t in visits_dict:
		...
		#polling can't exceed our 
			...
			m.addConstr(mu[(i,t)] * quicksum(city_per[city]* v[(city,t)] for city in cities[s]), GRB.LESS_EQUAL, diff)
			#m.addConstr(diff, GRB.LESS_EQUAL, -(mu[(i,t)]) * quicksum(city_per[city]* (1-v[(city,t)]) for city in cities[s]))

		else:
			diff = LinExpr(poll[(i,t)]-initpoll[s])		
			m.addConstr(mu[(i,t)] * quicksum(city_per[city]* v[(city,t)] for city in cities[s]), GRB.EQUAL, diff)
for t in visits_dict:
