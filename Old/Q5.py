...
for i, coord1 in coords.iteritems():
	for j, coord2 in coords.iteritems():
		if j == 1:		
			...
			v[i,j] = m.addVar(ub=1, obj=values[j], vtype=GRB.BINARY, name = "v"+points[i]+"-"+ points[j])		
...
balance.append(m.addConstr(0 == quicksum(v[1,j] for j in range(1,7)) - quicksum(v[j,1] for j in range(1,7)) ))
...
#	balance.append(m.addConstr(1 == quicksum(v[j,i] for j in range(1,7))))
	balance.append(m.addConstr(0 == quicksum(v[i,j] for j in range(1,7)) - quicksum(v[j,i] for j in range(1,7)) ))
